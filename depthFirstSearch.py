...
#getminDeptofAllneighbours
	...
	def findPathTestRun(self): 
		...
		qResult = queue.LifoQueue(maxsize=0)

		scanDepth = mazeDepth[x][y] #this is the depth to the observed cell.
		qtmp = queue.Queue()
		qtmp.put([0,0,scanDepth,[0]])
		...
		while(not qtmp.empty()): 
			...
			for currNode in neighbours:
				...
				if(currNode[0]==targetCell[0] and currNode[1]==targetCell[1]):	
					parentMap[(currNode[0],currNode[1])]=[node[0],node[1]]
					curr = currNode
					while (curr != [0,0]):
						#print (curr)
						qResult.put([curr[0],curr[1]])
						if len(parentMap[(curr[0],curr[1])]) >2:
							curr = parentMap[(curr[0],curr[1])][0]
						else:
